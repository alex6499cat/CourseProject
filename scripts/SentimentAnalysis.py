...
threads = pd.read_csv('threads.csv',nrows = 5000, dtype = {'tweet_id': str} )
...
full_df = pd.read_csv("twcs.csv", na_filter= False, parse_dates = ['created_at'],
                      dtype = {'tweet_id': str,'in_response_to_tweet_id': str, 'inbound':bool, 'response_tweet_id':str })
full_df.set_index("tweet_id", inplace = True)

inbound_col, tweet_col, first_sentim_col, last_sentim_col = thread_ok.columns.get_indexer(["inbound_l","tweet_l","first_sentim_l", "last_sentim_l"])
...
with CoreNLPClient(annotators=['sentiment'], 
    ...
    for row in range(len(thread_ok)):
        ...
        thread_ok.iloc[row,[first_sentim_col,last_sentim_col]]= ['|'.join(first_sentiment),'|'.join(last_sentiment)]
